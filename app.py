...
@st.cache_resource
def load_model():
    model = joblib.load("C:\\ML Projects\\Logical Regression\\logistic_regression_model.pkl")
    return model

# Load the dataset to get feature names
@st.cache_resource
def load_data():
    df = pd.read_csv("C:\\Users\\Pranav\\OneDrive\\Desktop\\ML Projects\\heart.csv.xls")
    return df
...
try:
    ...
    st.error(f"❌ Error: {e}")
    st.info("Make sure the model file is saved at: C:\\ML Projects\\Logical Regression\\logistic_regression_model.pkl")
